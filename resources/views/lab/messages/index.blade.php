                        ...
                        <div class="card-body">
                            <ul class="list-unstyled list-unstyled-border">
                                @foreach ($senders as $sender)
                                    ...
                                    @endphp
                                    <li class="media fp_chat_user" data-name="{{ $chatUser->first_name }}"
                                        data-user="{{ $chatUser->id }}" style="cursor: pointer">
                                        <img alt="image" class="mr-3 rounded-circle " width="50"
                                            src="{{ asset($chatUser->image) }}"
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            var image = "{{ asset(auth()->user()->image) }}";
            $('#receiver_id').val("");
            ...
            $('.fp_chat_user').on('click', function() {
                ...
                $.ajax({
                    method: 'GET',
                    url: '{{ route('lab.chat.get-conversation', ':senderId') }}'.replace(
                        ":senderId", senderId),
                    beforeSend: function() {
                        $('.chat-content').empty();
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, message) {
                            let image = "{{ asset(':image') }}".replace(':image',
                                message.sender.image);

                            let html = `
                            <div class="chat-item ${message.sender_id == userId ? "chat-right" : "chat-left"} " style=""><img style="width:50px;height:50px;object-fit:cover;" src="${image}"><div class="chat-details"><div class="chat-text">${message.message}</div></div></div>
                            `
            ...
            $('#chat-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        ...
                        let html = `
                            <div class="chat-item chat-right" style=""><img src="${image}"><div class="chat-details"><div class="chat-text">${message}</div><div class="chat-time ${msgId}">sending...</div></div></div>
                            `
