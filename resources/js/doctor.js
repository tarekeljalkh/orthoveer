...
window.Echo.private("chat."+loggedInUserId).listen(
    ...
    (e) => {
        ...
        if(e.senderId == $('#mychatbox').attr('data-inbox')){
            let html = `
            <div class="chat-item chat-left" style=""><img style="width:50px;height:50px;object-fit:cover;" src="${e.image}"><div class="chat-details"><div class="chat-text">${e.message}</div><div class="chat-time">sending...</div></div></div>
            `
